...
(async () => {
  const { generateCostWorkbook } = require('./src/lib/excel/generateCostSheet');
  ...
  const quote = {
    ...
    summary: { subtotal: 0, agencyFee: 0, vat: 0, total: 0 },
  };

  const buf = await generateCostWorkbook(quote, { productName: '테스트', bidCode: 'TEST' });
